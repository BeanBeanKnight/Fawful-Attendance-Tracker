...
from AttendanceTracker.Backend.enums.command_type import CommandType
from AttendanceTracker.Backend.helpers.array_helper import translate_user_info_to_display_strings, translate_command_array_to_display_strings
from AttendanceTracker.Backend.services.coin_bank_portal import CoinBankPortal
from AttendanceTracker.Backend.utilities.info_manager import InfoManager


def generate_leaderboard_reply(user_array: []):
    names, coin_counts = translate_user_info_to_display_strings(user_array)
    embed = discord.Embed()
    embed = discord.Embed(title="AttendanceTracker Leaderboard", color=0x0b1bf4)
    embed.add_field(name="Name", value=str(names), inline=True)
    embed.add_field(name="AttendanceTracker", value=str(coin_counts), inline=True)
    return embed
...
def generate_help_reply(command_descriptions: []):
    ...
    embed = discord.Embed()
    embed = discord.Embed(title="AttendanceTracker Commands", color=0x0b1bf4)
...
def generate_targeted_coin_count_reply(user_name, user_coin_count):
    if user_coin_count is not None:
        return f"{user_name} has {user_coin_count} AttendanceTracker in the bank"
    else:
        return f"{user_name} isn't set up in the bank and therefore has zero AttendanceTracker."
...
def generate_dank_hof_reply(target_user):
    return f"Because of the dankness of your dank af post, by the power invested in me I hereby bestow upon {target_user} ten AttendanceTracker! Dilly Dilly"
...
def generate_coin_count_updated_reply(target_user, new_value):
    return f"{target_user} now has {new_value} AttendanceTracker."
...
class ReplyGenerator:
    ...
    def generate_AttendanceTracker_ping_reply(self, targeted_user, amount):
        current_balance = self.info_manager.get_current_balance(targeted_user)
        if targeted_user is not None:
            if amount > 0:
                if amount == 1:
                    amount = "a"
                return f"Gave {targeted_user} {amount} AttendanceTracker! {targeted_user} now has {current_balance} in the bank."
            elif amount < 0:
                if amount == -1:
                    amount = "a"
                return f"{targeted_user} lost {amount} AttendanceTracker! {targeted_user} now has {current_balance} in the bank."
        return None
